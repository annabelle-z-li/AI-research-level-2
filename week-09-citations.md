...
All four Week 7 sources are carried forward into this citation file — none were dropped at the formatting stage. However, verification is not complete until you have clicked each DOI and confirmed it resolves to the correct paper, and until you have located each supporting passage in the actual PDF (page or section number noted). The Bereket & Shi source has no DOI and should be clearly labeled as non-peer-reviewed in the brief. The note-fading passage from Section 7 is not included above as a cited claim — you discussed it in your reading notes but should decide whether it belongs in the brief as a second citation from that source or stays as background. One source from the Week 7 near-miss list — the 2024 MT3 follow-up (YourMT3+) — may be worth revisiting if you need a more current reference for the state-of-the-art gap argument, but only if you can verify the claim and find the supporting passage.
